...
func makeHandleRepositoryGet(t *testing.T) func(http.ResponseWriter, *http.Request) {
	if t == nil {
		...
		return func(w http.ResponseWriter, r *http.Request) {
			repo_regex := "^" + pathRepository + "(/.+){0,2}$"
			request_path := r.URL.Path
			matchSimple, _ := regexp.MatchString(repo_regex, pathRepository)
			matchUserParam, _ := regexp.MatchString(repo_regex, pathRepository+"/user")
			matchUserParamRepoParam, _ := regexp.MatchString(repo_regex, pathRepository+"/user/foo")
			if !(matchSimple || matchUserParam || matchUserParamRepoParam) {
				t.Errorf("Regex Match Fail")
			}
			if match, _ := regexp.MatchString(repo_regex, request_path); !match {
			...
			w.Write([]byte(sample_response_repo))
		}
	}
}
...
func TestGitHubRepositories(t *testing.T) {
	...
	if got == nil {
		...
		if *got.ID != 1296269 || *got.Owner.Login != "octocat" {
			t.Errorf("Mock response failed")
		}
	}
}

const sample_response_repo string = `{
    "id": 1296269,
